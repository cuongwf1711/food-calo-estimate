...
class SegmentationModel:
    ...
    def __init__(self):
        ...
        self.SAM2_PREDICTOR = SAM2ImagePredictor(build_sam2(SAM2_CONFIG, SAM2_CHECKPOINT, device=DEVICE))
    ...
    def _get_boxes_groundino(self, image):
        """Get boxes."""
        inputs = self.processor(images=image, text=TEXT_PROMPT_LIST, return_tensors="pt").to(
            DEVICE
        )
    ...
    def _get_boxes_dinox(self, image):
        ...
        bboxes, labels = zip(*((obj["bbox"], obj["category"]) for obj in task.result["objects"]))
